...
def load_twitter_data(path_dir):
    ...
    # hyperedge_file_path = osp.join(path_dir, "hyperedges_updated.txt")
    # H = np.loadtxt(hyperedge_file_path)
    # row, col = H.nonzero()
    # values = np.ones(col.shape)
    # matrix = csr_matrix((values, (row, col)), shape=H.shape)
